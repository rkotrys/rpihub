...
if( isset($_GET['set'] ) and $_GET['set']!='' ){
   switch ( $_GET['set'] ){
      ...
      case 'btdiscover':
         $rpi=$db->get($_GET['sn']);
         if( is_array($rpi) and count($rpi)>0 ){   
            $cmd=base64_encode( json_encode( array( 'name'=>'btdiscover', 'value'=>'', 'sn'=>$rpi['sn'] ) ) );
            $db->update( array( 'sn'=>$rpi['sn'], 'cmd'=>$cmd ) );
         }
      header("Location: /?get=getall");
      exit;
      ...
      case 'btdiscoveroff':
         $rpi=$db->get($_GET['sn']);
         if( is_array($rpi) and count($rpi)>0 ){   
            $cmd=base64_encode( json_encode( array( 'name'=>'btdiscoveroff', 'value'=>'', 'sn'=>$rpi['sn'] ) ) );
            $db->update( array( 'sn'=>$rpi['sn'], 'cmd'=>$cmd ) );
         }
      header("Location: /?get=getall");
      exit;
      ...
      case 'btscan':
         $rpi=$db->get($_GET['sn']);
         if( is_array($rpi) and count($rpi)>0 ){   
            $cmd=base64_encode( json_encode( array( 'name'=>'btscan', 'value'=>'', 'sn'=>$rpi['sn'] ) ) );
            $db->update( array( 'sn'=>$rpi['sn'], 'cmd'=>$cmd ) );
         }
      header("Location: /?get=getall");
      exit;
      ...
      case 'btremove':
         $rpi=$db->get($_GET['sn']);
         if( is_array($rpi) and count($rpi)>0 ){   
            $cmd=base64_encode( json_encode( array( 'name'=>'btremove', 'value'=>$_GET['btremove'], 'sn'=>$rpi['sn'] ) ) );
            $db->update( array( 'sn'=>$rpi['sn'], 'cmd'=>$cmd ) );
         }
      header("Location: /?get=getall");
      exit;
      ...
      case 'towlanBridgeAP':
         $rpi=$db->get($_GET['sn']);
         if( is_array($rpi) and count($rpi)>0 ){   
            $cmd=base64_encode( json_encode( array( 'name'=>'towlanBridgeAP', 'sn'=>$rpi['sn'] ) ) );
            $db->update( array( 'sn'=>$rpi['sn'], 'cmd'=>$cmd ) );
         }
      header("Location: /?get=getall");         
      exit;
